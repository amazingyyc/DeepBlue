...
{
	//
	//LOGD("(w, h, r):%d, %d, %d", w, h, radius);
	////////////////////////////////////////////////////////

	float sigma =  1.0 * radius / 3.0;	//3sigam半径之后基本没有贡献 所以取sigma为 r / 3
...
void gaussBlur(int* pix, int w, int h, int r)
{
	float sigma = 1.0 * r / 2.57;	//3sigam半径之后基本没有贡献 所以取sigma为 r / 2.57
...
void Java_com_amazing_utils_GaussBlur_initCBlur(JNIEnv* env,
												jobject obj,
												jintArray pix,
												jint w,
												jint h,
												jint r)
{
	gaussBlur(env->GetIntArrayElements(pix, NULL), w, h, r);
}

void Java_com_amazing_utils_GaussBlur_initCBlur2(JNIEnv* env,
												jobject obj,
												jintArray pix,
												jint w,
												jint h,
